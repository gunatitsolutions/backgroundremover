...
class FlutterBackgroundRemover {
  ...
  static Future<img.Image> _removeBackgroundFromImage(
    ...
    final int maskWidth = segmentationMask.width;
    final int maskHeight = segmentationMask.height;

    // Resize the mask to match the original image size if needed
    final img.Image newImage = img.Image(width: width, height: height);

    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        final int mx = (x * maskWidth / width).floor().clamp(0, maskWidth - 1);
        final int my = (y * maskHeight / height).floor().clamp(0, maskHeight - 1);
        final int maskIndex = my * maskWidth + mx;

        final double confidence =
        segmentationMask.confidences[maskIndex].clamp(0.0, 1.0);

        final int alpha = (confidence * 255).toInt();

        final originalPixel = image.getPixel(x, y);
        final int r = originalPixel.r.toInt();
        final int g = originalPixel.g.toInt();//img.getGreen(originalPixel);
        final int b = originalPixel.b.toInt();//img.getBlue(originalPixel);

        // Apply soft alpha mask based on person confidence
